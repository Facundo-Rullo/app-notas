...
export async function POST(request) {
    ...
    try {
        ...
        const notasFormateadas = notes
        .map(n => `-Titulo: ${n.title}\n contenido: ${n.content}`)
        .join("\n\n")

        const prompCompleto = `
        Eres "Asistant IA" un modelo IA avanzado integrado en un sistema de notas y documentacion sobre desarrollo.
        Responde de forma sumamente tecnia a las dudas del usuario.
        Ve directo al grano.
        ...
        const aiText = data.candidates?.[0]?.content?.parts?.[0]?.text || "No pude generar respuesta"
